...
\vspace{-.3cm}
\begin{list1}
\item[] {\em NLP Research Scientist} \hfill {\em July 2017 - May 2019}\\
  \vspace{-.3cm}
\end{list1}
...
c.execute('SELECT title, authors, month, year, link FROM technical_report ORDER BY year DESC, month DESC')
for row in c:
  title, authors, month, year, link = row

  if month is not None:
    dateobj = datetime.date(2000, int(month), 1)
    month = dateobj.strftime('%b')
  
  print(r'\item[] {\em %s.} {\bf %s.} %s %d.\\' % (', '.join(authors.split('|')), title, month, year))
